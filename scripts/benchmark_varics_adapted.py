...
if __name__ == '__main__':
    for condition, (dictionary_path, model_path) in conditions.items():
        print(condition)
        output_model_path = acoustic_model_directory.joinpath(f'varics_{condition}_adapted_base.zip')
        output_directory = os.path.join(root_directory, 'varics_alignments', f'{condition}_adapted_base')
        if not output_model_path.exists():
            ...
            command = ['adapt',
                       ...
                       '--use_reference_alignments', 'false'
                       ]
            print(command)
